    ...
    if((H5Tget_class(tid) == H5T_COMPOUND)
        ...
        if(H5Tequal(tid, H5T_STD_REF_OBJ) > 0) {
            ...
            for(u = 0; u < nelmts; u++, ref_buf1++, ref_buf2++) {
                ...
                if(H5Rget_obj_type3(*ref_buf1, &obj1_type) < 0) TEST_ERROR
                if(H5Rget_obj_type3(*ref_buf2, &obj2_type) < 0) TEST_ERROR
                if(obj1_type != obj2_type) TEST_ERROR

                /* Open referenced objects */
                if((obj1_id = H5Ropen_object(*ref_buf1, H5P_DEFAULT)) < 0) TEST_ERROR
                if((obj2_id = H5Ropen_object(*ref_buf2, H5P_DEFAULT)) < 0) TEST_ERROR
            ...
            for(u = 0; u < nelmts; u++, ref_buf1++, ref_buf2++) {
                ...
                if((obj1_sid = H5Ropen_region(*ref_buf1)) < 0) TEST_ERROR
                if((obj2_sid = H5Ropen_region(*ref_buf2)) < 0) TEST_ERROR
...
test_copy_null_ref(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl, hid_t dst_fapl)
{
    ...
    /* Create references */
    if(H5Rcreate_object(did1, ".", &obj_buf[0]) < 0) TEST_ERROR
    if(H5Rcreate_object(did2, ".", &obj_buf[1]) < 0) TEST_ERROR
    if(H5Rcreate_region(did1, ".", sid, &reg_buf[0]) < 0) TEST_ERROR
    if(H5Rcreate_region(did2, ".", sid, &reg_buf[1]) < 0) TEST_ERROR
...
test_copy_null_ref_open(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl, hid_t dst_fapl)
{
    ...
    href_t obj_buf[3];                          /* Buffer for object refs */
    href_t reg_buf[3];                          /* Buffer for region refs */
    char zeros[MAX(sizeof(obj_buf),sizeof(reg_buf))]; /* Array of zeros, for memcmp */
    char src_filename[NAME_BUF_SIZE];
    char mid_filename[NAME_BUF_SIZE];
    char dst_filename[NAME_BUF_SIZE];
    ...
    if(H5Dread(did3, H5T_STD_REF_OBJ, H5S_ALL, H5S_ALL, H5P_DEFAULT, obj_buf)
        < 0) TEST_ERROR
    if(H5Dread(did4, H5T_STD_REF_REG, H5S_ALL, H5S_ALL, H5P_DEFAULT, reg_buf)
        < 0) TEST_ERROR
    ...
    if(H5Dread(did5, H5T_STD_REF_OBJ, H5S_ALL, H5S_ALL, H5P_DEFAULT, obj_buf)
        < 0) TEST_ERROR
    if(H5Dread(did6, H5T_STD_REF_REG, H5S_ALL, H5S_ALL, H5P_DEFAULT, reg_buf)
        < 0) TEST_ERROR
