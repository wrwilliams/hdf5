...
test_refer_dtype(hid_t file)
{
    ...
    if(H5Rcreate_object(file, "/Group1/Datatype1", &wref) < 0)
        TEST_ERROR;
    if(H5Rget_obj_type3(wref, &obj_type) < 0)
        TEST_ERROR;
    ...
    if((tid1 = H5Ropen_object(rref, H5P_DEFAULT)) < 0)
        TEST_ERROR;
...
test_refer_dtype2(hid_t file)
{
    ...
    if(H5Rcreate_region(file, "/Dataset2", sid2, &wref) < 0)
        TEST_ERROR;
    if(H5Rget_obj_type3(wref, &obj_type) < 0)
        TEST_ERROR;
    ...
    /* Try to open objects */
    if((dset2 = H5Ropen_object(rref, H5P_DEFAULT)) < 0)
        TEST_ERROR;

    /* Check what H5Rget_obj_type3 function returns */
    if(H5Rget_obj_type3(rref, &obj_type) < 0)
        TEST_ERROR;
    if(obj_type != H5O_TYPE_DATASET)
        TEST_ERROR;
