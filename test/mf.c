...
/*
 *  For now: this only works for old format.
 *  For new format: paged_aggregation, due to !allow_small_shrink, the section is not shrink away and that
 *              causes assertion failure in H5FScache.c due: 
 *                  hdr_preserialize() has 1 section count with NULL sect_info,
 *                  then it calls H5MF_alloc() to allocate section hdr will cause the section count to 0.
 *                  Then in hdr_serialize(), the assertion will fail because sect_hdr is nonnull but section count is 0.
 *                  Discuss with John M about this, this will need to be addressed when we come to page_buffering.
 */
...
test_page_alloc_xfree(const char *env_h5_drvr, hid_t fapl)
{
    ...
    for(fs_persist = FALSE; fs_persist <= TRUE; fs_persist++) {
        ...
        if(!fs_persist) {
            ...
            if(H5MF_find_sect(f, H5FD_MEM_OHDR, H5AC_ind_read_dxpl_id, (hsize_t)TBLOCK_SIZE1034, f->shared->fs.man[fs_type], &found_addr) < 0)
                TEST_ERROR
            if(found_addr != addr2)
                TEST_ERROR
        } /* end if */
        ...
        if(!fs_persist) {
            ...
            if(H5MF_find_sect(f, H5FD_MEM_DRAW, H5AC_ind_read_dxpl_id, (hsize_t)TBLOCK_SIZE30, f->shared->fs.man[H5F_MEM_PAGE_DRAW], &found_addr) < 0)
                TEST_ERROR
        ...
        if(!fs_persist) {
            ...
            if(H5MF_find_sect(f, H5FD_MEM_DRAW, H5AC_ind_read_dxpl_id, (hsize_t)TBLOCK_SIZE8192, f->shared->fs.man[fs_type], &found_addr) < 0)
                TEST_ERROR
        ...
        if(fs_persist) {
            ...
            H5MF_alloc_to_fs_type(f, H5FD_MEM_OHDR, f->shared->fs.page_size - 1, (H5F_mem_page_t *)&fs_type);
            ...
            if(!H5F_addr_defined(f->shared->fs.man_addr[H5F_MEM_PAGE_DRAW]))
                TEST_ERROR
        
            /* Set up to use the small raw data manager */
            if(!(f->shared->fs.man[H5F_MEM_PAGE_DRAW]))
...
test_page_track_last_small(const char *env_h5_drvr, hid_t fapl)
{
    ...
    hid_t fid = -1;                 /* File ID */
    hid_t fapl_new = -1;            /* File access property list */
    hid_t fcpl = -1;                /* File creation property list */
    H5F_t *f = NULL;                /* Internal file object pointer */
    ...
    if(f->shared->fs.track_last_small != H5F_MEM_PAGE_DRAW)
        TEST_ERROR
    ...
    if(!multi && !split)
        ...
        if(f->shared->fs.track_last_small != H5F_MEM_PAGE_META)
            TEST_ERROR
...
test_page_try_shrink(const char *env_h5_drvr, hid_t fapl)
{
    ...
    H5MF_xfree(f, H5FD_MEM_OHDR, H5AC_ind_read_dxpl_id, addr1, (hsize_t)TBLOCK_SIZE50);
...
test_page_small(const char *env_h5_drvr, hid_t fapl)
{
    ...
    /* Current VFD that does not support continuous address space */
    contig_addr_vfd = (hbool_t)(HDstrcmp(env_h5_drvr, "multi") && HDstrcmp(env_h5_drvr, "split"));
...
main(void)
{
    ...
    nerrors += test_page_track_last_small(env_h5_drvr, fapl);   /* handle multi/split */
    nerrors += test_page_alloc_xfree(env_h5_drvr, fapl);        /* handle multi/split */
    nerrors += test_page_alignment(env_h5_drvr, fapl);          /* handle multi/split */
