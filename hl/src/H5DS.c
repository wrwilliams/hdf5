                        ...
                        unsigned int idx)
{
    ...
    */
    ...
    if (H5Rcreate_object(dsid, ".", &ref_to_ds) < 0)
        return FAIL;

    /* create a reference for the >>data<< dataset */
    if (H5Rcreate_object(did, ".", &dsl.ref) < 0)
        return FAIL;
    ...
    */
    else if(has_dimlist == 1)
    {
        ...
        for(i = 0; i < (int)buf[idx].len; i++) {
            ...
            if((dsid_j = H5Ropen_object(ref_j, H5P_DEFAULT)) < 0)
                goto out;
    ...
    */
    ...
    if ( buf[idx].len > 0 )
    {
        for (j=0; j<buf[idx].len; j++)
        {
            ...
            /* get the DS id */
            if ((dsid_j = H5Ropen_object(ref, H5P_DEFAULT)) < 0)
                goto out;
    ...
    */
    ...
    for(ii=0; ii<nelmts; ii++) {
      ...
      if(idx == dsbuf[ii].dim_idx) {
            ...
            /* get the dataset id */
            if ((did_i = H5Ropen_object(ref, H5P_DEFAULT)) < 0)
                goto out;
    ...
    else if(has_dimlist == 1)
    {
        ...
        if ( buf[dim].len > 0 )
        {
            ...
            for(i=j_idx; i<nscales; i++)
            {
                ...
                H5E_BEGIN_TRY {
                    /* get the DS id */
                    if ((scale_id = H5Ropen_object(ref, H5P_DEFAULT)) < 0)
                        goto out;
                } H5E_END_TRY;
