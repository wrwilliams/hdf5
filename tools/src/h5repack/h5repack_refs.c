...
static herr_t update_ref_value(href_t ref_in, hid_t fid_out, href_t *ref_out,
                               trav_table_t *travt);
            ...
            case H5TRAV_TYPE_DATASET:
                ...
                if(h5tools_canreadf(NULL, dcpl_id) == 1) {
                    ...
                    if(H5Tequal(mtype_id, H5T_STD_REF_OBJ)) {
                        hid_t            refobj_id = -1;
                        href_t          *refbuf = NULL; /* buffer for object references */
                        href_t          *buf = NULL;
                        const char*      refname;
                        unsigned         u;
                         ...
                         */
                        if(nelmts) {
                            buf = (href_t *)HDmalloc((unsigned)(nelmts * msize));
                            if(buf==NULL) {
                            ...
                            refbuf = (href_t *)HDcalloc((unsigned)nelmts, msize);
                            if(refbuf == NULL){
                            ...
                            for(u = 0; u < nelmts; u++) {
                                H5E_BEGIN_TRY {
                                    if((refobj_id = H5Ropen_object(buf[u], H5P_DEFAULT)) < 0)
                                        continue;
                                } H5E_END_TRY;
                                ...
                                if((refname = MapIdToName(refobj_id, travt)) != NULL) {
                                    ...
                                    if(H5Rcreate_object(fidout, refname, &refbuf[u]) < 0)
                                        HGOTO_ERROR(FAIL, H5E_tools_min_id_g, "H5Rcreate_object failed");
                                    if(options->verbose) {
                                ...
                                if (H5Rdestroy(buf[u]) < 0)
                                    H5TOOLS_INFO(H5E_tools_min_id_g, "H5Rdestroy failed");
                            } /* u */
                        } /*nelmts*/
                        ...
                        if(refbuf) {
                            for(u = 0; u < nelmts; u++)
                                if (H5Rdestroy(refbuf[u]) < 0)
                                    HGOTO_ERROR(FAIL, H5E_tools_min_id_g, "H5Rdestroy failed");
                            HDfree(refbuf);
                        }
                     ...
                     *-------------------------------------------------------------------------
                     */
                    else if(H5Tequal(mtype_id, H5T_STD_REF_REG)) {
                        hid_t            refobj_id = -1;
                        href_t          *refbuf = NULL; /* input buffer for region references */
                        href_t          *buf = NULL;    /* output buffer */
                        const char*      refname;
                        unsigned         u;
                        ...
                        if(nelmts) {
                             ...
                             *-------------------------------------------------------------------------
                             */
                            refbuf = (href_t *)HDcalloc(sizeof(href_t), (size_t)nelmts); /*init to zero */
                            if(refbuf == NULL) {
                                printf("cannot allocate memory\n");
                                HGOTO_ERROR(FAIL, H5E_tools_min_id_g, "HDcalloc failed");
                            } /* end if */
                            ...
                            for(u = 0; u < nelmts; u++) {
                                ...
                                if((refname = MapIdToName(refobj_id, travt)) != NULL) {
                                    ...
                                    if((region_id = H5Ropen_region(buf[u])) < 0)
                                        HGOTO_ERROR(FAIL, H5E_tools_min_id_g, "H5Ropen_region failed");

                                    /* create the reference, we need the space_id */
                                    if(H5Rcreate_region(fidout, refname, region_id, &refbuf[u]) < 0)
                                        HGOTO_ERROR(FAIL, H5E_tools_min_id_g, "H5Rcreate_region failed");
        ...
        if (nelmts>0) {
            ...
            if(is_ref || is_ref_array) {
                ...
                for(i = 0; i < (unsigned)nelmts; i++) {
                    if (update_ref_value(((href_t *)buf)[i], fidout, &refbuf[i], travt)<0)
                        continue;
                    if(options->verbose)
                        printf("object <%s> region reference created to <%s>\n", name, refname);
                }
                ...
                for (i=0; i<(unsigned)nelmts; i++) {
                    href_t *ptr = (href_t *)((hvl_t *)buf)[i].p;

                    for (j=0; j<((hvl_t *)buf)[i].len; j++ ) {
                        if (update_ref_value(ptr[j], fidout, &ref_out, travt)<0)
                            continue;
                ...
                for (i=0; i<(unsigned)nelmts; i++) {
                    for (j=0; j<(unsigned)ref_comp_field_n; j++) {
                        ...
                        if (update_ref_value(*((href_t *)(((char *)buf)+idx)), fidout, &ref_out, travt)<0)
                            continue;
    ...
    trav_table_t *travt)
{
    ...
    H5R_type_t ref_type;
    href_t new_ref = HREF_NULL;

    ref_type = H5Rget_type(ref_in);
    if (ref_type == H5R_BADTYPE)
        HGOTO_ERROR(FAIL, H5E_tools_min_id_g, "Bad reference type");

    ref_obj_id = H5Ropen_object(ref_in, H5P_DEFAULT);
    if (ref_obj_id < 0)
        HGOTO_ERROR(FAIL, H5E_tools_min_id_g, "H5Ropen_object failed");
    ...
    if (ref_type == H5R_REGION) {
        space_id = H5Ropen_region(ref_in);
        if (space_id < 0)
            HGOTO_ERROR(FAIL, H5E_tools_min_id_g, "H5Rget_region2 failed");
        if (H5Rcreate_region(fid_out, ref_obj_name, space_id, &new_ref) < 0)
            HGOTO_ERROR(FAIL, H5E_tools_min_id_g, "H5Rcreate_region failed");
    } else if (ref_type == H5R_OBJECT) {
        if (H5Rcreate_object(fid_out, ref_obj_name, &new_ref) < 0)
            HGOTO_ERROR(FAIL, H5E_tools_min_id_g, "H5Rcreate_object failed");
    }
