...
static void gent_objref(void)
{
    ...
    H5Rcreate_object(fid1, "/Group1/Dataset1", &wbuf[0]);

    /* Create reference to dataset */
    H5Rcreate_object(fid1, "/Group1/Dataset2", &wbuf[1]);

    /* Create reference to group */
    H5Rcreate_object(fid1, "/Group1", &wbuf[2]);

    /* Create reference to named datatype */
    H5Rcreate_object(fid1, "/Group1/Datatype1", &wbuf[3]);
...
static void gent_datareg(void)
{
    ...
    H5Rcreate_region(fid1, "/Dataset2", sid2, &wbuf[0]);
    ...
    H5Rcreate_region(fid1, "/Dataset2", sid2, &wbuf[1]);
...
static void gent_array1_big(void)
{
    ...
    H5Rcreate_region(fid1, "/Dataset1", sid1, &wbuf[0]);
        ...
        hid_t fid)
{
    ...
    if(dset_name)
    {
        H5Rcreate_object(fid, dset_name, &buf4[0]);
        H5Rcreate_object(fid, dset_name, &buf4[1]);
    ...
    if(dset_name)
    {
        for(i = 0; i < 3; i++) {
            for(j = 0; j < 2; j++) {
                H5Rcreate_object(fid, dset_name, &buf42[i][j]);
            }
        }
    ...
    if(dset_name)
    {
        for(i = 0; i < 4; i++) {
            for(j = 0; j < 3; j++) {
                for(k = 0; k < 2; k++)
                    H5Rcreate_object(fid, dset_name, &buf43[i][j][k]);
            }
        }
