	   ...
	   int compressed, int latest)
{
    ...
    for (j=0; j<ndsets; j+=12) {
		...
		/* 11 add object refs */
		H5Rcreate_object(gid1, ".", &buf_ref[0]);
		H5Rcreate_object(gid1, tmp_name3, &buf_ref[1]);
		...
		H5Rcreate_region(gid1, tmp_name1, sid_2d, &buf_reg_ref[0]);
		H5Sselect_none(sid_2d);
		count = dims[0]/2+1;
		H5Sselect_hyperslab (sid_1d, H5S_SELECT_SET, &start, &stride, &count,NULL);
		H5Rcreate_region(gid1, tmp_name2, sid_1d, &buf_reg_ref[1]);
