...
int make_deflate(hid_t loc_id)
{
    ...
    */
    ...
    if (H5Rcreate_object(loc_id, "dset_deflate", &bufref[0]) < 0)
        goto out;
                   ...
                   int make_diffs /* flag to modify data buffers */)
{
    ...
    */
    ...
    if (dset_name)
    {
        if (H5Rcreate_object(file_id,dset_name, &buf4[0]) < 0)
            goto out;
    ...
    */
    ...
    if (dset_name)
    {
        if (H5Rcreate_object(file_id, dset_name, &buf42[0][0]) < 0)
            goto out;
    ...
    */
    ...
    if (dset_name)
    {
        if (H5Rcreate_object(file_id, dset_name, &buf43[0][0][0]) < 0)
            goto out;
...
int make_dset_reg_ref(hid_t loc_id)
{
    ...
    if (H5Rcreate_region(loc_id, "dsetreg", sid2, &wbuf[0]) < 0)
        goto out;
                   ...
                   int make_diffs /* flag to modify data buffers */)
{
    ...
    */
    ...
    if (dset_name)
    {
        if (H5Rcreate_object(fid,dset_name, &buf4[0]) < 0)
            goto out;
        if (H5Rcreate_object(fid,dset_name, &buf4[1]) < 0)
            goto out;
    ...
    */
    ...
    if (dset_name)
    {
        for (i = 0; i < 3; i++)
            for (j = 0; j < 2; j++)
                if (H5Rcreate_object(fid,dset_name, &buf42[i][j]) < 0)
                    goto out;
    ...
    */
    ...
    if (dset_name)
    {
        for (i = 0; i < 4; i++)
            for (j = 0; j < 3; j++)
                for (k = 0; k < 2; k++)
                    if (H5Rcreate_object(fid,dset_name, &buf43[i][j][k]) < 0)
                        goto out;
...
static herr_t add_attr_with_objref(hid_t file_id, hid_t obj_id)
{
     ...
     */
    /* ref to dset */
    status = H5Rcreate_object(file_id,NAME_OBJ_DS1, &data_attr_objref[0]);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_object failed.\n", FUNC, __LINE__);
        ret = FAIL;
        goto out;
    }

    /* ref to group */
    status = H5Rcreate_object(file_id,NAME_OBJ_GRP, &data_attr_objref[1]);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_object failed.\n", FUNC, __LINE__);
        ret = FAIL;
    ...
    /* ref to datatype */
    status = H5Rcreate_object(file_id,NAME_OBJ_NDTYPE, &data_attr_objref[2]);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_object failed.\n", FUNC, __LINE__);
        ret = FAIL;
...
static herr_t add_attr_with_regref(hid_t file_id, hid_t obj_id)
{
    ...
    status = H5Rcreate_region(file_id, NAME_OBJ_DS2, sid_regrefed_dset, &data_attr_regref[0]);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_region failed.\n", FUNC, __LINE__);
        ret = FAIL;
        goto out;
    }
...
static herr_t gen_obj_ref(hid_t loc_id)
{
    ...
    /* obj ref to dataset */
    status = H5Rcreate_object (loc_id, NAME_OBJ_DS1, &objref_buf[0]);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_object failed.\n", FUNC, __LINE__);
        ret = FAIL;
    ...
    /* obj ref to group */
    status = H5Rcreate_object (loc_id, NAME_OBJ_GRP, &objref_buf[1]);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_object failed.\n", FUNC, __LINE__);
        ret = FAIL;
    ...
    status = H5Rcreate_object (loc_id, NAME_OBJ_NDTYPE, &objref_buf[2]);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_object failed.\n", FUNC, __LINE__);
        ret = FAIL;
...
static herr_t gen_region_ref(hid_t loc_id)
{
    ...
    /* create region reference from elements space */
    status = H5Rcreate_region (loc_id, NAME_OBJ_DS2, sid_trg, &rr_data[0]);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_region failed.\n", FUNC, __LINE__);
        ret = FAIL;
    ...
    status = H5Rcreate_region (loc_id, NAME_OBJ_DS2, sid_trg, &rr_data[1]);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_region failed.\n", FUNC, __LINE__);
        ret = FAIL;
...
static herr_t make_complex_attr_references(hid_t loc_id)
{
    ...
    status = H5Rcreate_object (loc_id, NAME_OBJ_DS1, &comp_objref_data[0].val_objref);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_object failed.\n", FUNC, __LINE__);
        ret = FAIL;
    ...
    comp_objref_data[0].val_int = 0;
     ...
     /* references to group */
    status = H5Rcreate_object (loc_id, NAME_OBJ_GRP, &comp_objref_data[1].val_objref);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_object failed.\n", FUNC, __LINE__);
        ret = FAIL;
    ...
    status = H5Rcreate_object (loc_id, NAME_OBJ_NDTYPE, &comp_objref_data[2].val_objref);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_object failed.\n", FUNC, __LINE__);
        ret = FAIL;
    ...
    status = H5Rcreate_region (loc_id, NAME_OBJ_DS1, objsid, &comp_regref_data[0].val_regref);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_region failed.\n", FUNC, __LINE__);
        ret = FAIL;
     ...
     */
     ...
     status = H5Rcreate_object(loc_id, NAME_OBJ_DS1, vlen_objref_data_ptr[0]);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_object failed.\n", FUNC, __LINE__);
        ret = FAIL;
     ...
     /* reference to group */
    status = H5Rcreate_object(loc_id, NAME_OBJ_GRP, vlen_objref_data_ptr[1]);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_object failed.\n", FUNC, __LINE__);
        ret = FAIL;
    ...
    status = H5Rcreate_object(loc_id, NAME_OBJ_NDTYPE, vlen_objref_data_ptr[2]);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_object failed.\n", FUNC, __LINE__);
        ret = FAIL;
    ...
    status = H5Rcreate_region(loc_id, NAME_OBJ_DS1, objsid, vlen_regref_data_ptr[0]);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_region failed.\n", FUNC, __LINE__);
        ret = FAIL;
