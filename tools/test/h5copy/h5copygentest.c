...
static herr_t gen_obj_ref(hid_t loc_id)
{
    ...
    status = H5Rcreate_object (loc_id, OBJ_REF_DS, &or_data[0]);
    if (status < 0)
    {
    ...
    status = H5Rcreate_object (loc_id, OBJ_REF_GRP, &or_data[1]);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_object failed.\n", FUNC, __LINE__);
        ret = FAIL;
...
static herr_t gen_region_ref(hid_t loc_id)
{
    ...
    status = H5Rcreate_region (loc_id, REG_REF_DS2, sid, &rr_data[0]);
    if (status < 0)
    {
    ...
    status = H5Rcreate_region (loc_id, REG_REF_DS2, sid, &rr_data[1]);
    if (status < 0)
    {
        fprintf(stderr, "Error: %s %d> H5Rcreate_region failed.\n", FUNC, __LINE__);
        ret = FAIL;
