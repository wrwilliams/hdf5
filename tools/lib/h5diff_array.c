        ...
        else if (!iszero1 && !iszero2)
        {
            ...
            */
            if (ref_type==H5R_REGION)
            {
                ...
                if ((obj1_id = H5Ropen_object(ref1, H5P_DEFAULT))<0) {
                    opts->err_stat = 1;
                    H5TOOLS_INFO(H5E_tools_min_id_g, "H5Ropen_object object 1 failed");
                }
                if ((obj2_id = H5Ropen_object(ref2, H5P_DEFAULT))<0) {
                    opts->err_stat = 1;
                    H5TOOLS_INFO(H5E_tools_min_id_g, "H5Ropen_object object 2 failed");
                }
                if ((region1_id = H5Ropen_region(ref1))<0) {
                    opts->err_stat = 1;
                    H5TOOLS_INFO(H5E_tools_min_id_g, "H5Ropen_region object 1 failed");
                }
                if ((region2_id = H5Ropen_region(ref2))<0) {
                    opts->err_stat = 1;
                    H5TOOLS_INFO(H5E_tools_min_id_g, "H5Ropen_region object 2 failed");
                }
            ...
            */
            else if (ref_type==H5R_OBJECT)
            {
                ...
                if (H5Rget_obj_type3(ref1, &obj1_type) < 0) {
                    opts->err_stat = 1;
                    H5TOOLS_INFO(H5E_tools_min_id_g, "H5Rget_obj_type3 object 1 failed");
                }
                if (H5Rget_obj_type3(ref2, &obj2_type) < 0) {
                    opts->err_stat = 1;
