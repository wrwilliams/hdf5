...
static herr_t H5T__ref_mem_read(H5F_t *f, void *_ref, void *_buf, size_t buf_size);
static herr_t H5T__ref_mem_write(H5F_t *f, void *_ref, void *_buf, void *_bg, size_t buf_size, H5R_type_t ref_type);

static size_t H5T__obj_ref_mem_getsize(const void *_ref);
static herr_t H5T__obj_ref_mem_read(H5F_t *f, void *_ref, void *_buf, size_t buf_size);
static herr_t H5T__obj_ref_mem_write(H5F_t *f, void *_ref, void *_buf, void *_bg, size_t buf_size, H5R_type_t ref_type);

static size_t H5T__ref_disk_getsize(const void *_ref);
static herr_t H5T__ref_disk_read(H5F_t *f, void *_ref, void *_buf, size_t buf_size);
static herr_t H5T__ref_disk_write(H5F_t *f, void *_ref, void *_buf, void *_bg, size_t buf_size, H5R_type_t ref_type);

static size_t H5T__obj_ref_disk_getsize(const void *_ref);
static herr_t H5T__obj_ref_disk_read(H5F_t *f, void *_ref, void *_buf, size_t buf_size);
static herr_t H5T__obj_ref_disk_write(H5F_t *f, void *_ref, void *_buf, void *_bg, size_t buf_size, H5R_type_t ref_type);
...
H5T__ref_mem_getsize(const void *_ref)
{
    ...
    /* Add for passing around file ID */
    FUNC_LEAVE_NOAPI(ref->ref.serial.buf_size + sizeof(hid_t))
 ...
 */
static herr_t
H5T__ref_mem_read(H5F_t H5_ATTR_UNUSED *f, void *_ref, void *buf,
    size_t H5_ATTR_UNUSED buf_size)
{
    const href_t *ref_ptr = (const href_t *)_ref;
    struct H5Tconv_ref *conv_ref = (struct H5Tconv_ref *) buf;
    ...
    conv_ref->file_id = ref->loc_id;
    HDmemcpy(&conv_ref->serial, ref->ref.serial.buf, ref->ref.serial.buf_size);
 ...
 */
static herr_t
H5T__ref_mem_write(H5F_t H5_ATTR_UNUSED *f, void *_ref, void *buf,
    void H5_ATTR_UNUSED *_bg, size_t buf_size, H5R_type_t ref_type)
{
    ...
    /* Set type etc */
    ref->loc_id = conv_ref->file_id;
    ref->ref_type = ref_type;
    ref->ref.serial.buf_size = buf_size - offsetof(struct H5Tconv_ref, serial);
    ...
    HDmemcpy(ref->ref.serial.buf, &conv_ref->serial, ref->ref.serial.buf_size);
...
H5T__obj_ref_mem_getsize(const void H5_ATTR_UNUSED *_ref)
{
    ...
    FUNC_LEAVE_NOAPI(sizeof(struct H5Tconv_obj_ref))
 ...
 */
static herr_t
H5T__obj_ref_mem_read(H5F_t H5_ATTR_UNUSED *f, void *_ref, void *buf,
    size_t H5_ATTR_UNUSED buf_size)
{
    const href_t *ref_ptr = (const href_t *)_ref;
    struct H5Tconv_obj_ref *conv_ref = (struct H5Tconv_obj_ref *) buf;
    const struct href *ref;
    ...
    conv_ref->obj_addr = ref->ref.addr;
 ...
 */
static herr_t
H5T__obj_ref_mem_write(H5F_t H5_ATTR_UNUSED *f, void *_ref, void *buf,
    void H5_ATTR_UNUSED *_bg, size_t H5_ATTR_UNUSED buf_size, H5R_type_t ref_type)
{
    herr_t ret_value = SUCCEED; /* Return value */
    href_t *ref_ptr = (href_t *)_ref;
    struct H5Tconv_obj_ref *conv_ref = (struct H5Tconv_obj_ref *) buf;
    ...
    HDassert(ref_ptr);
    HDassert(buf);
    HDassert(buf_size == sizeof(struct H5Tconv_obj_ref));
    ...
    /* Copy the data into the newly allocated buffer */
    HDmemcpy(&ref->loc_id, &conv_ref->file_id, sizeof(hid_t));
    HDmemcpy(&ref->ref.addr, &conv_ref->obj_addr, sizeof(ref->ref.addr));
 ...
 */
static herr_t
H5T__ref_disk_read(H5F_t *f, void *_ref, void *buf,
    size_t H5_ATTR_UNUSED buf_size)
{
    ...
    /* Get file ID */
    if((conv_ref->file_id = H5F_get_id(f)) < 0)
    ...
    if(H5HG_read(f, &hobjid, &conv_ref->serial, NULL) == NULL)
 ...
 */
static herr_t
H5T__ref_disk_write(H5F_t *f, void *_ref, void *buf, void *_bg, size_t buf_size,
    H5R_type_t H5_ATTR_UNUSED ref_type)
{
    ...
    /* Set the size */
    UINT32ENCODE(ref, buf_size - offsetof(struct H5Tconv_ref, serial));

    /* Write the reference information to disk (allocates space also) */
    if(H5HG_insert(f, buf_size - offsetof(struct H5Tconv_ref, serial), &conv_ref->serial, &hobjid) < 0)
 ...
 */
static herr_t
H5T__obj_ref_disk_read(H5F_t H5_ATTR_UNUSED *f, void *_ref, void *buf,
    size_t H5_ATTR_UNUSED buf_size)
{
    struct H5Tconv_obj_ref *conv_ref = (struct H5Tconv_obj_ref *) buf;
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    if((conv_ref->file_id = H5F_get_id(f)) < 0)
        HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL, "can't get file ID")
    HDmemcpy(&conv_ref->obj_addr, _ref, sizeof(haddr_t));
 ...
 */
static herr_t
H5T__obj_ref_disk_write(H5F_t H5_ATTR_UNUSED *f, void *_ref, void *buf,
    void H5_ATTR_UNUSED *_bg, size_t H5_ATTR_UNUSED buf_size,
    H5R_type_t H5_ATTR_UNUSED ref_type)
{
    struct H5Tconv_obj_ref *conv_ref = (struct H5Tconv_obj_ref *) buf;
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    HDmemcpy(_ref, &conv_ref->obj_addr, sizeof(haddr_t));
