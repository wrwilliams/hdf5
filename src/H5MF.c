...
HDfprintf(stderr, "%s: adding node, node->sect_info.addr = %a, node->sect_info.size = %Hu\n", FUNC, node->sect_info.addr, node->sect_info.size);
    ...
    if(H5FS_sect_add(f, dxpl_id, fspace, (H5FS_section_info_t *)node, H5FS_ADD_RETURNED_SPACE, &udata) < 0)
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTINSERT, FAIL, "can't re-add section to file free space")
...
H5MF_find_sect(H5F_t *f, H5FD_mem_t alloc_type, hid_t dxpl_id, hsize_t size, H5FS_t *fspace, haddr_t *addr)
{
    ...
    if((ret_value = H5FS_sect_find(f, dxpl_id, fspace, size, (H5FS_section_info_t **)&node)) < 0)
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTALLOC, FAIL, "error locating free space in file")
...
HDfprintf(stderr, "%s: freeing node\n", FUNC);
            ...
            if(H5MF_sect_free((H5FS_section_info_t *)node) < 0)
                HGOTO_ERROR(H5E_FSPACE, H5E_CANTRELEASE, FAIL, "can't free simple section node")
        } /* end if */
        else {
...
HDfprintf(stderr, "%s: alloc_type = %u, size = %Hu\n", FUNC, (unsigned)alloc_type, size);
    ...
    if(f->shared->first_alloc_dealloc) {
        ...
        if(H5MF_tidy_self_referential_fsm_hack(f, dxpl_id) < 0)
            HGOTO_ERROR(H5E_FSPACE, H5E_CANTFREE, HADDR_UNDEF, "tidy of self referential fsm hack failed")
    } /* end if */
...
HDfprintf(stderr, "%s: Check 1.0\n", FUNC);
    ...
    if(H5F_HAVE_FREE_SPACE_MANAGER(f)) {
        ...
        if(H5AC_unsettle_ring(f, fsm_ring) < 0)
            HGOTO_ERROR(H5E_FSPACE, H5E_SYSTEM, HADDR_UNDEF, "attempt to notify cache that ring is unsettled failed")
        ...
        if(f->shared->fs_man[fs_type])
            if(H5MF_find_sect(f, alloc_type, dxpl_id, size, f->shared->fs_man[fs_type], &ret_value) < 0)
                HGOTO_ERROR(H5E_FSPACE, H5E_CANTALLOC, HADDR_UNDEF, "error locating a node")
...
HDfprintf(stderr, "%s: Check 2.0\n", FUNC);
        ...
        if(f->shared->fs_strategy == H5F_FSPACE_STRATEGY_PAGE) {
            ...
            if(HADDR_UNDEF == (ret_value = H5MF__alloc_pagefs(f, alloc_type, dxpl_id, size)))
                HGOTO_ERROR(H5E_FSPACE, H5E_CANTALLOC, HADDR_UNDEF, "allocation failed from paged aggregation")
        } /* end if */
    ...
    switch(ptype) {
	    ...
	    case H5F_MEM_PAGE_LARGE_OHDR:
        {
            ...
            if(HADDR_UNDEF == (ret_value = H5F_alloc(f, dxpl_id, alloc_type, size + frag_size, NULL, NULL)))
                HGOTO_ERROR(H5E_FSPACE, H5E_CANTALLOC, HADDR_UNDEF, "can't allocate file space")
            ...
            if(frag_size) {
                ...
                if(!(f->shared->fs_man[ptype]))
                    if(H5MF_start_fstype(f, dxpl_id, ptype) < 0)
                        HGOTO_ERROR(H5E_FSPACE, H5E_CANTINIT, HADDR_UNDEF, "can't initialize file free space")

                /* Create free space section for the fragment */
                if(NULL == (node = H5MF_sect_new(H5MF_FSPACE_SECT_LARGE, ret_value + size, frag_size)))
                    HGOTO_ERROR(H5E_FSPACE, H5E_CANTINIT, HADDR_UNDEF, "can't initialize free space section")
        ...
        case H5F_MEM_PAGE_OHDR:
        {
            ...
            if(f->shared->page_buf != NULL && H5PB_add_new_page(f, alloc_type, new_page) < 0)
                HGOTO_ERROR(H5E_FSPACE, H5E_CANTINSERT, HADDR_UNDEF, "can't add new page to Page Buffer new page list")
        ...
        default:
            HGOTO_ERROR(H5E_FSPACE, H5E_CANTALLOC, HADDR_UNDEF, "can't allocate file space: unrecognized type")
            break;
    } /* end switch */
...
H5MF_sects_dump(f, dxpl_id, stderr);
    ...
    if(node)
        if(H5MF_sect_free((H5FS_section_info_t *)node) < 0)
            HDONE_ERROR(H5E_FSPACE, H5E_CANTRELEASE, HADDR_UNDEF, "can't free section node")
...
HDfprintf(stderr, "%s: Entering - alloc_type = %u, addr = %a, size = %Hu\n", FUNC, (unsigned)alloc_type, addr, size);
    ...
    if(f->shared->first_alloc_dealloc){
        HDassert(!H5AC_cache_image_pending(f));
        if(H5MF_tidy_self_referential_fsm_hack(f, dxpl_id) < 0)
            HGOTO_ERROR(H5E_FSPACE, H5E_CANTFREE, FAIL, "tidy of self referential fsm hack failed")
    } /* end if */
    ...
    if(H5F_HAVE_FREE_SPACE_MANAGER(f))
        if(H5AC_unsettle_ring(f, fsm_ring) < 0)
            HGOTO_ERROR(H5E_FSPACE, H5E_SYSTEM, FAIL, "attempt to notify cache that ring is unsettled failed")
    ...
    if(f->shared->first_alloc_dealloc) {
        if(H5AC_cache_image_pending(f)) {
            if(H5AC_force_cache_image_load(f, dxpl_id) < 0)
                HGOTO_ERROR(H5E_FSPACE, H5E_CANTFREE, FAIL, "forced cache image load failed")
        } /* end if */
        else {
            if(H5MF_tidy_self_referential_fsm_hack(f, dxpl_id) < 0)
                HGOTO_ERROR(H5E_FSPACE, H5E_CANTFREE, FAIL, "tidy of self referential fsm hack failed")
...
H5MF_settle_raw_data_fsm(H5F_t *f, hid_t dxpl_id)
{
    ...
    if(H5F_super_ext_write_msg(f, dxpl_id, H5O_FSINFO_ID, &fsinfo, TRUE, H5O_MSG_FLAG_MARK_IF_UNKNOWN) < 0)
        HGOTO_ERROR(H5E_FSPACE, H5E_WRITEERROR, FAIL, "error in writing fsinfo message to superblock extension")
...
H5MF_settle_meta_data_fsm(H5F_t *f, hid_t dxpl_id)
{
    ...
    if(H5F_PAGED_AGGR(f)) {
        ...
        if(lg_hdr_fspace) {
            ...
            if(H5FS_stat_info(f, lg_hdr_fspace, &fs_stat) < 0)
                HGOTO_ERROR(H5E_FSPACE, H5E_CANTGET, FAIL, "can't get free-space info (3)")
        ...
        if((lg_sinfo_fspace) && (lg_hdr_fspace != lg_sinfo_fspace)) {
            ...
            if(H5FS_stat_info(f, lg_sinfo_fspace, &fs_stat) < 0)
                HGOTO_ERROR(H5E_FSPACE, H5E_CANTGET, FAIL, "can't get free-space info (4)")
    ...
    if(sm_hdr_fspace) {
        if(H5FS_vfd_alloc_hdr_and_section_info_if_needed(f, dxpl_id,
            ...
            HGOTO_ERROR(H5E_FSPACE, H5E_CANTALLOC, FAIL, "can't vfd allocate sm hdr FSM file space")
    }
    ...
    if((sm_sinfo_fspace) && (sm_sinfo_fspace != sm_hdr_fspace)) {
        if(H5FS_vfd_alloc_hdr_and_section_info_if_needed(f, dxpl_id,
            ...
            HGOTO_ERROR(H5E_FSPACE, H5E_CANTALLOC, FAIL, "can't vfd allocate sm sinfo FSM file space")
    }
    ...
    if ( H5F_PAGED_AGGR(f) ) {
        if ( lg_hdr_fspace ) {
            if ( H5FS_vfd_alloc_hdr_and_section_info_if_needed(f, dxpl_id,
                ...
                HGOTO_ERROR(H5E_FSPACE, H5E_CANTALLOC, FAIL, "can't vfd allocate lg hdr FSM file space")
        }
        ...
        if ( ( lg_sinfo_fspace ) && ( lg_sinfo_fspace != lg_hdr_fspace ) ) {
            if ( H5FS_vfd_alloc_hdr_and_section_info_if_needed(f, dxpl_id,
                ...
                HGOTO_ERROR(H5E_FSPACE, H5E_CANTALLOC, FAIL, "can't vfd allocate lg sinfo FSM file space")
        }
    }
...
H5MF_tidy_self_referential_fsm_hack(H5F_t *f, hid_t dxpl_id)
{
    ...
    if(H5F_PAGED_AGGR(f)) {
        if(f->shared->fs_man[lg_fshdr_fs_type]) {
            ...
            if(H5FS_free(f, f->shared->fs_man[lg_fshdr_fs_type], dxpl_id, FALSE) < 0)
                HGOTO_ERROR(H5E_FSPACE, H5E_CANTRELEASE, FAIL, "can't float free-space headers")
    ...
    if(H5F_free(f, dxpl_id, H5FD_MEM_DEFAULT, f->shared->eoa_pre_fsm_fsalloc, tail_size) < 0)
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTFREE, FAIL, "driver free request failed")
    if(HADDR_UNDEF == (eoa = H5FD_get_eoa(f->shared->lf, H5FD_MEM_DEFAULT)))
        HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL, "unable to get EOA")
