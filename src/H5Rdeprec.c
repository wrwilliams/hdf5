...
H5Rcreate(void *_ref, hid_t loc_id, const char *name, H5R_type_t ref_type, hid_t space_id)
{
    ...
    switch (ref_type) {
        case H5R_OBJECT:
            if(NULL == (ref = H5R_create_object(&loc, name, H5AC_ind_read_dxpl_id)))
        ...
        case H5R_REGION:
            ...
            if(NULL == (ref = H5R_create_region(&loc, name, H5AC_ind_read_dxpl_id, space)))
                HGOTO_ERROR(H5E_REFERENCE, H5E_CANTINIT, FAIL, "unable to create object reference")
            break;
        case H5R_ATTR:
        case H5R_EXT_OBJECT:
...
H5Rdereference1(hid_t obj_id, H5R_type_t ref_type, const void *_ref)
{
    ...
    /* Dereference */
    if((ret_value = H5R__dereference(file, H5P_DATASET_ACCESS_DEFAULT, H5AC_ind_read_dxpl_id, *ref_ptr, TRUE)) < 0)
...
H5Rdereference2(hid_t obj_id, hid_t oapl_id, H5R_type_t ref_type, const void *_ref)
{
    ...
    /* Dereference */
    if((ret_value = H5R__dereference(loc.oloc->file, oapl_id, H5AC_ind_read_dxpl_id, *ref_ptr, TRUE)) < 0)
        HGOTO_ERROR(H5E_REFERENCE, H5E_CANTINIT, FAIL, "unable dereference object")
...
H5Rget_region1(hid_t dataset, H5R_type_t ref_type, const void *_ref)
{
    ...
    if((space = H5R__get_region(loc.oloc->file, H5AC_ind_read_dxpl_id, *ref_ptr)) == NULL)
...
H5Rget_obj_type1(hid_t id, H5R_type_t ref_type, const void *_ref)
{
    ...
    if(H5R__get_obj_type(loc.oloc->file, H5AC_ind_read_dxpl_id, *ref_ptr, &obj_type) < 0)
    ...
    char *name/*out*/, size_t size)
{
    ...
    switch (ref_type) {
        ...
        case H5R_DATASET_REGION:
        case H5R_REGION:
            if((ret_value = H5R__get_obj_name(file, H5P_DEFAULT, H5AC_ind_read_dxpl_id, loc_id, *ref_ptr, name, size)) < 0)
