    ...
    MPI_Aint *disp, MPI_Datatype *new_type) 
{
    ...
    if(H5S_MAX_MPI_COUNT >= num_points) {
      /* If num_points fit into an int, we can proceed by creating a single hindexed datatype */
#if MPI_VERSION >= 3
      ...
      /* LARGE_DATATYPE::
       * OTHERWISE we create an Hindexed_block type for every 2G point count,
     ...
     MPI_Aint *inner_disps;
        ...
        leftover = (hsize_t)num_points - (hsize_t)num_big_types * (hsize_t)H5S_MAX_MPI_COUNT;
    ...
    hbool_t *is_permuted)
{
    ...
    for(u = 0 ; u < num_points ; u++) {
        ...
        disp[u] =  (MPI_Aint)H5VM_array_offset(space->extent.rank, space->extent.size, curr->pnt);
...
*******************************************************/
    for(i = ((int)rank) - 1; i >= 0; --i) {
       ...
       ****************************************/
       ...
       start_disp = (MPI_Aint)(d[i].start * offset[i] * elmt_size);
...
***************************/
    ...
    *new_type = inner_type;
    ...
    /* only have to move one of these suckers! */ 
    *count = 1;
    *is_derived_type = TRUE;
    HGOTO_DONE(SUCCEED);
    ...
    const MPI_Datatype *elmt_type, MPI_Datatype *span_type, size_t elmt_size)
{
    ...
    if(NULL == span->down) {
        ...
        /* LARGE_DATATYPE:: Something doesn't fit into a 32 bit integer */
        else {
            size_t i;
                                          ...
                                          MPI_Datatype *new_type)
{
    ...
    if(remaining_bytes) {
        ...
        block_len[1] = 1;
        disp[0] = 0;
        disp[1] = (old_extent+stride_bytes)*num_big_types*(MPI_Aint)H5S_MAX_MPI_COUNT;
