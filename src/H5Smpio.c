...
static herr_t H5S__mpio_span_hyper_type(const H5S_t *space, size_t elmt_size,
    MPI_Datatype *new_type, int *count, hbool_t *is_derived_type);
static herr_t H5S__release_datatype(H5S_hyper_span_info_t *spans, uint64_t op_gen);
    ...
    MPI_Datatype *new_type, int *count, hbool_t *is_derived_type)
{
    ...
    if(H5S__obtain_datatype(space->select.sel_info.hslab->span_lst, down, elmt_size, &elmt_type, &span_type, op_gen) < 0)
    ...
    /* Acquire a new operation generation value for releasing MPI datatypes */
    op_gen = H5S__hyper_get_op_gen();

    /* Release MPI data types for span trees */
    if(H5S__release_datatype(space->select.sel_info.hslab->span_lst, op_gen) < 0)
 ...
 *
 * Purpose:	Release the MPI derived datatype for span-tree hyperslab selection
 *
 * Return:	Non-negative on success, negative on failure.
 *
 ...
 */
static herr_t
H5S__release_datatype(H5S_hyper_span_info_t *spans, uint64_t op_gen)
{
    herr_t ret_value = SUCCEED;         /* Return value */
    ...
    if(spans->op_gen != op_gen) {
        ...
        /* If this is not the fastest changing dimension, release lower dimension MPI data types */
        if(NULL != spans->head->down) {
            ...
            while(span) {
                ...
                if(H5S__release_datatype(span->down, op_gen) < 0)
                    HGOTO_ERROR(H5E_DATASPACE, H5E_BADTYPE, FAIL, "couldn't release MPI derived data type")
        ...
        if(MPI_SUCCESS != (mpi_code = MPI_Type_free(&spans->u.down_type)))
