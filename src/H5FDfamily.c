...
H5FD_family_sb_decode(H5FD_t *_file, const char UNUSED *name, const unsigned char *buf)
{
    ...
    if(msize != file->pmem_size) {
        ...
        HDsnprintf(err_msg, (size_t)128, "Family member size should be %lu.  But the size from file access property is %lu", (unsigned long)msize, (unsigned long)file->pmem_size);
		 ...
		 haddr_t maxaddr)
{
    ...
    HDsnprintf(memb_name, (size_t)4096, name, 0);
    HDsnprintf(temp, (size_t)4096, name, 1);
    if(!HDstrcmp(memb_name, temp))
        HGOTO_ERROR(H5E_FILE, H5E_FILEEXISTS, NULL, "file names not unique")

    /* Open all the family members */
    while(1) {
        HDsnprintf(memb_name, (size_t)4096, name, file->nmembs);
...
H5FD_family_set_eoa(H5FD_t *_file, H5FD_mem_t type, haddr_t abs_eoa)
{
    ...
    for(u = 0; addr || u < file->nmembs; u++) {
        ...
        if(u >= file->nmembs || !file->memb[u]) {
            ...
            HDsnprintf(memb_name, (size_t)4096, file->name, u);
            H5E_BEGIN_TRY {
