...
H5Rcreate(void *ref, H5R_type_t ref_type, ...)
{
    ...
    switch (ref_type) {
        case H5R_OBJECT:
        {
            ...
            if((ret_value = H5R_create(ref, ref_type, &loc, name, H5AC_ind_read_dxpl_id)) < 0)
        ...
        case H5R_DATASET_REGION:
        {
            ...
            if((ret_value = H5R_create(ref, ref_type, &loc, name, H5AC_ind_read_dxpl_id, space)) < 0)
                HGOTO_ERROR(H5E_REFERENCE, H5E_CANTINIT, FAIL, "unable to create reference")
        }
            break;
        case H5R_ATTR:
        {
            ...
            if((ret_value = H5R_create(ref, ref_type, &loc, name, H5AC_ind_read_dxpl_id, attr_name)) < 0)
                HGOTO_ERROR(H5E_REFERENCE, H5E_CANTINIT, FAIL, "unable to create reference")
        }
            break;
        case H5R_MAXTYPE:
...
H5Rdereference2(hid_t obj_id, hid_t oapl_id, H5R_type_t ref_type, const void *_ref)
{
    ...
    /* Create reference */
    if((ret_value = H5R_dereference(loc.oloc->file, oapl_id, dxpl_id, ref_type, _ref, TRUE)) < 0)
...
H5Rget_region(hid_t id, H5R_type_t ref_type, const void *ref)
{
    ...
    if((space = H5R_get_region(loc.oloc->file, H5AC_ind_read_dxpl_id, ref_type, ref)) == NULL)
