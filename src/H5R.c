...
H5Rcreate(void *ref, hid_t loc_id, const char *name, H5R_type_t ref_type, ...)
{
    ...
    switch (ref_type) {
        case H5R_OBJECT:
            if((ret_value = H5R_create(ref, &loc, name, H5AC_ind_read_dxpl_id, ref_type)) < 0)
        ...
        case H5R_DATASET_REGION:
        {
            ...
            if((ret_value = H5R_create(ref, &loc, name, H5AC_ind_read_dxpl_id, ref_type, space)) < 0)
                HGOTO_ERROR(H5E_REFERENCE, H5E_CANTINIT, FAIL, "unable to create reference")
        }
            break;
        case H5R_ATTR:
        {
            ...
            if((ret_value = H5R_create(ref, &loc, name, H5AC_ind_read_dxpl_id, ref_type, attr_name)) < 0)
                HGOTO_ERROR(H5E_REFERENCE, H5E_CANTINIT, FAIL, "unable to create reference")
        }
            break;
        case H5R_MAXTYPE:
