...
int main(void)
{
    ...
    H5Rcreate_region(file_id, dsetnamev, space_id, &ref[0]);
    ...
    H5Rcreate_region(file_id, dsetnamev, space_id, &ref[1]);

    /*
     * Write dataset with the references.
     */
    H5Dwrite(dsetr_id, H5T_STD_REF_REG, H5S_ALL, H5S_ALL, H5P_DEFAULT,ref);
    ...
    H5Dread(dsetr_id, H5T_STD_REF_REG, H5S_ALL, H5S_ALL, H5P_DEFAULT, ref_out);
     ...
     * using H5Rget_name
     */
    name_size1 = H5Rget_obj_name(ref_out[0], (char*)buf1, 10);
    ...
    space_id = H5Ropen_region(ref_out[0]);

    /*
     * Read and display hyperslab selection from the dataset.
     */

    H5Dread(dsetv_id, H5T_NATIVE_INT, H5S_ALL, space_id, H5P_DEFAULT, data_out);
     ...
     */
    dsetv_id = H5Ropen_object(ref_out[1], H5P_DEFAULT);
    space_id = H5Ropen_region(ref_out[1]);
    ...
    H5Dclose(dsetv_id);
    H5Dclose(dsetr_id);
    H5Rdestroy(ref_out[0]);
    H5Rdestroy(ref_out[1]);
    H5Fclose(file_id);
