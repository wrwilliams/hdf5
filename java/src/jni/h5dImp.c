          ...
          jlong space_id, jlong create_plist_id)
{
    ...
    file = ENVPTR->GetStringUTFChars(ENVPAR name, &isCopy);
    if (file == NULL) {
  ...
  (JNIEnv *env, jclass clss, jstring dir_name)
{
    ...
    file = ENVPTR->GetStringUTFChars(ENVPAR dir_name, &isCopy);
    if (file == NULL) {
  ...
  (JNIEnv *env, jclass clss, jobjectArray dir_name, jint buf_size)
{
    ...
    aName = (char*)malloc(sizeof(char) * (size_t)buf_size);
    if (aName == NULL) {
        h5outOfMemory( env, "H5Dgetcwd:  malloc failed");
        return -1;
    }
    JGETCWD( (char*)aName, (size_t)buf_size);
 ...
 * Signature: (J[B)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dextend
          ...
          jlong file_space_id, jlong xfer_plist_id, jobjectArray j_buf)
{
    ...
    if ((c_buf = (char*)malloc((size_t)n * str_len)) == NULL) {
        if (cstr)
            free (cstr);
        cstr = NULL;
        h5JNIFatalError(env, "H5Dread_string: memory allocation failed.");
 ...
 * Signature: (JJJJJ[Ljava/lang/String;)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5DwriteString
  (JNIEnv *env, jclass clss, jlong dataset_id, jlong mem_type_id, jlong mem_space_id,
          jlong file_space_id, jlong xfer_plist_id, jobjectArray buf)
{
    ...
    wdata = (char**)malloc((size_t)size * sizeof (char*));
 ...
 * Signature: (JJJJJ[BZ)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5DwriteNotString
  (JNIEnv *env, jclass clss, jlong dataset_id, jlong mem_type_id, jlong mem_space_id,
          jlong file_space_id, jlong xfer_plist_id, jbyteArray buf, jboolean isCriticalPinning)
{
    herr_t  status;
    ...
    wdata = (hvl_t*)calloc((size_t)n, sizeof(hvl_t));
    ...
    memset(wdata, 0, (size_t)n * sizeof(hvl_t));
    ...
    hid_t file_sid, hid_t xfer_plist_id, jobjectArray buf)
{
    ...
    rdata = (hvl_t*)calloc((size_t)n, sizeof(hvl_t));
    if (rdata == NULL) {
    ...
    file_sid, hid_t xfer_plist_id, jobjectArray buf)
{
    ...
    n = ENVPTR->GetArrayLength(ENVPAR buf);
    strs =(char**)calloc((size_t)n, sizeof(char*));
 ...
 * Signature: (JJJJJ[Ljava/lang/String;)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dread_1reg_1ref (JNIEnv *env, jclass clss,
        ...
        jlong file_space_id, jlong xfer_plist_id, jobjectArray buf)
{
    ...
    ref_data = (hdset_reg_ref_t*)malloc(size * (size_t)n);
 ...
 * Signature: (JJJJJ[Ljava/lang/String;)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dread_1reg_1ref_1data (JNIEnv *env, jclass clss,
        jlong dataset_id, jlong mem_type_id, jlong mem_space_id,
        jlong file_space_id, jlong xfer_plist_id, jobjectArray buf)
{
    herr_t    status;
  ...
  (JNIEnv *env, jclass clss, jlong loc_id, jlongArray buf)
{
    ...
    dims = (hsize_t*) malloc((size_t)rank * sizeof(hsize_t));
    for (i = 0; i< rank; i++)
